...
function estimate_wind_2d,curr,prev,wind_prior,search_size,n,apa,actlocs,maxit,LINEAR = linear,NOMASK = nomask
	...
	if mask_edges then begin
		curr = twod(curr[actlocs],n,actlocs)
	endif
...
;	set up shift vectors to use for interpolation
	...
	if cubic_interp then begin
		shift_points[*,*,0] = interpolate(prev,ix_cen,iy_pls,cubic = -0.5,/grid)
		shift_points[*,*,1] = interpolate(prev,ix_min,iy_cen,cubic = -0.5,/grid)
		shift_points[*,*,2] = interpolate(prev,ix_pls,iy_cen,cubic = -0.5,/grid)
		shift_points[*,*,3] = interpolate(prev,ix_cen,iy_min,cubic = -0.5,/grid)
	endif else begin
	;create shifted images using linear interpolation
		shift_points[*,*,0] = interpolate(prev,ix_cen,iy_pls,/grid)
		shift_points[*,*,1] = interpolate(prev,ix_min,iy_cen,/grid)
		shift_points[*,*,2] = interpolate(prev,ix_pls,iy_cen,/grid)
		shift_points[*,*,3] = interpolate(prev,ix_cen,iy_min,/grid)
	endelse

	for j=0,maxit-1 do begin
		;mask out edges of shifted images
		shift_points[*,*,0] = twod((shift_points[*,*,0])[actlocs],n,actlocs)
		shift_points[*,*,1] = twod((shift_points[*,*,1])[actlocs],n,actlocs)
		shift_points[*,*,2] = twod((shift_points[*,*,2])[actlocs],n,actlocs)
		shift_points[*,*,3] = twod((shift_points[*,*,3])[actlocs],n,actlocs)


		;find min abs error
		err0 = total(abs(shift_points[*,*,0] - curr))
		err1 = total(abs(shift_points[*,*,1] - curr))
		err2 = total(abs(shift_points[*,*,2] - curr))
		err3 = total(abs(shift_points[*,*,3] - curr))
...
;		err3 = total((shift_points[*,*,3] - curr)^2)
		...
		case aa of
			0:	begin
					...
					if cubic_interp then begin
						shift_points[*,*,0] = interpolate(prev,ix_cen,iy_pls,/grid,cubic = -0.5)
						shift_points[*,*,1] = interpolate(prev,ix_min,iy_cen,/grid,cubic = -0.5)
						shift_points[*,*,2] = interpolate(prev,ix_pls,iy_cen,/grid,cubic = -0.5)
						shift_points[*,*,3] = interpolate(prev,ix_cen,iy_min,/grid,cubic = -0.5)
					endif else begin
						;create shifted images using linear interpolation
						shift_points[*,*,0] = interpolate(prev,ix_cen,iy_pls,/grid)
						shift_points[*,*,1] = interpolate(prev,ix_min,iy_cen,/grid)
