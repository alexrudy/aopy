...
function estimate_wind_2d_x,curr,prev,wind_prior,search_size,n,apa,actlocs,maxit,LINEAR = linear,NOMASK = nomask
...
;	set up shift vectors to use for interpolation
	...
	if cubic_interp then begin
		shift_points[*,*,0] = interpolate(prev,ix_min,iy_cen,/grid,cubic = -0.5)
		shift_points[*,*,1] = interpolate(prev,ix_pls,iy_cen,/grid,cubic = -0.5)
	endif else begin
	;create shifted images using linear interpolation
		shift_points[*,*,0] = interpolate(prev,ix_min,iy_cen,/grid)
		shift_points[*,*,1] = interpolate(prev,ix_pls,iy_cen,/grid)
	endelse
