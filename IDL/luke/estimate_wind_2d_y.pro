...
function estimate_wind_2d_y,curr,prev,wind_prior,search_size,n,apa,actlocs,maxit,LINEAR = linear,NOMASK = nomask
...
;	set up shift vectors to use for interpolation
	iy_pls = findgen(n) - u_x[1] - search_size
	ix_cen = findgen(n) - u_x[0]
	iy_min = findgen(n) - u_x[1] + search_size
	...
	if cubic_interp then begin
		shift_points[*,*,0] = interpolate(prev,ix_cen,iy_min,/grid,cubic = -0.5)
		shift_points[*,*,1] = interpolate(prev,ix_cen,iy_pls,/grid,cubic = -0.5)
	endif else begin
	;create shifted images using linear interpolation
		shift_points[*,*,0] = interpolate(prev,ix_cen,iy_min,/grid)
		shift_points[*,*,1] = interpolate(prev,ix_cen,iy_pls,/grid)
	endelse
