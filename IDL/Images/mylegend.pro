...
;;; 8-30-2010 DOH! IDL 8.0 now has it's own legend procedure
;which mangles this one!!!! ARGH!!!!!!
;+

;;; renaming this to mylegend instead of legend
...
;       MYLEGEND     
; PURPOSE:
;	This procedure makes a mylegend for a plot.  The mylegend can contain
;	a mixture of symbols, linestyles, Hershey characters (vectorfont),
;	and filled polygons (usersym).  A test procedure, mylegendtest.pro,
;	shows mylegend's capabilities.  Placement of the mylegend is controlled
...
; CALLING SEQUENCE:
;      	MYLEGEND [,items][,keyword options]
; EXAMPLES:
;	The call:
;		mylegend,['Plus sign','Asterisk','Period'],psym=[1,2,3]
;	  produces:
;		-----------------
;		|		|
;		|  + Plus sign	|
...
;	mylegend,items,linestyle=lines		; vertical mylegend---upper left
;	items = ['Plus sign','Asterisk','Period']
;	sym = [1,2,3]
;	mylegend,items,psym=sym			; ditto except using symbols
;	mylegend,items,psym=sym,/horizontal	; horizontal format
;	mylegend,items,psym=sym,box=0		; sans border
;	mylegend,items,psym=sym,delimiter='='	; embed '=' betw psym & text
;	mylegend,items,psym=sym,margin=2		; 2-character margin
;	mylegend,items,psym=sym,position=[x,y]	; upper left in data coords
;	mylegend,items,psym=sym,pos=[x,y],/norm	; upper left in normal coords
;	mylegend,items,psym=sym,pos=[x,y],/device	; upper left in device coords
;	mylegend,items,psym=sym,/position		; interactive position
;	mylegend,items,psym=sym,/right		; at upper right
;	mylegend,items,psym=sym,/bottom		; at lower left
;	mylegend,items,psym=sym,/center		; approximately near center
;	mylegend,items,psym=sym,number=2		; plot two symbols, not one
;	mylegend,items,/fill,psym=[8,8,8],colors=[10,20,30]; 3 filled squares
; INPUTS:
;	items = text for the items in the mylegend, a string array.
...
;	/horizontal = flag to make the mylegend horizontal
;	/vertical = flag to make the mylegend vertical (D=vertical)
;	box = flag to include/omit box around the mylegend (D=include)
;	clear = flag to clear the box area before drawing the mylegend 
...
;	position = data coordinates of the /top (D) /left (D) of the mylegend
...
;	/left = flag to place mylegend snug against left side of plot window (D)
;	/right = flag to place mylegend snug against right side of plot window
;		If /right,pos=[x,y], then x is position of RHS and text
;		runs right-to-left.
;	/top = flag to place mylegend snug against top of plot window (D)
;	/bottom = flag to place mylegend snug against bottom of plot window
...
;	symbol in the mylegend.
; Outputs:
;	mylegend to current plot device
; OPTIONAL OUTPUT KEYWORDS: 
;	corners = 4-element array, like !p.position, of the normalized
;	  coords for the box (even if box=0): [llx,lly,urx,ury].
;	  Useful for multi-column or multi-line mylegends, for example,
;	  to make a 2-column mylegend, you might do the following:
;	    c1_items = ['diamond','asterisk','square']
;	    c1_psym = [4,2,6]
;	    c2_items = ['solid','dashed','dotted']
;	    c2_line = [0,2,1]
;	    mylegend,c1_items,psym=c1_psym,corners=c1,box=0
;	    mylegend,c2_items,line=c2_line,corners=c2,box=0,pos=[c1(2),c1(3)]
;	    c = [c1(0)<c2(0),c1(1)<c2(1),c1(2)>c2(2),c1(3)>c2(3)]
;	    plots,[c(0),c(0),c(2),c(2),c(0)],[c(1),c(3),c(3),c(1),c(1)],/norm
;	  Useful also to place the mylegend.  Here's an automatic way to place
;	  the mylegend in the lower right corner.  The difficulty is that the
;	  mylegend's width is unknown until it is plotted.  In this example,
;	  the mylegend is plotted twice: the first time in the upper left, the
;	  second time in the lower right.
;	    mylegend,['1','22','333','4444'],linestyle=indgen(4),corners=corners
;			; BOGUS MYLEGEND---FIRST TIME TO REPORT CORNERS
...
;	    mylegend,['1','22','333','4444'],linestyle=indgen(4),pos=pos
;			; REDO THE MYLEGEND IN LOWER RIGHT CORNER
;	  You can modify the pos calculation to place the mylegend where you
...
;	mylegend.  The main problem with placement is not being
...
;	- An orientation keyword would allow lines at angles in the mylegend.
...
;	allow a mylegend with just text, no plotting symbols.  This helps in
...
;-
pro mylegend,help=help,items,linestyle=linestyle,psym=psym,vectorfont=vectorfont $
...
;
on_error,2
if keyword_set(help) then begin & doc_library,'mylegend' & return & endif
;
...
if n eq 0 then message,'No inputs!  For help, type mylegend,/help.'
if ni eq 0 then begin
  items = replicate('',n)				; DEFAULT BLANK ARRAY
endif else begin
  szt = size(items)
  if (szt(szt(0)+1) ne 7) then message,'First parameter must be a string array.  For help, type mylegend,/help.'
...
if n_elements(textcolors) eq 0 then textcolors = !color + intarr(n) else $
...
case n_elements(position) of
 ...
 else: message,'Position keyword can have 0, 1, or 2 elements only. Try mylegend,/help.'
endcase 
...
;
;	=====>> OUTPUT TEXT FOR MYLEGEND, ITEM BY ITEM.
